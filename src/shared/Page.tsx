  ...
  return (
    <header tw="max-w-5xl mx-auto px-4 sm:px-6 mt-6">
      <div tw="xl:flex xl:justify-between">
        ...
        {children && (
          <div tw="flex flex-row flex-wrap md:mt-0 md:ml-4 space-x-3 space-y-1 items-center">
            {children}
          </div>
        )}
      </div>
...
export const PageTitle = ({ as: Component = "h1", ...props }) => {
  return (
    <Component
      tw="text-3xl leading-9 font-extrabold text-gray-900"
      {...props}
    />
  );
};
    ...
    <Page>
      <PageHeader
        title={
          <PageTitle
            as="div"
            tw="h-8 my-1 bg-gray-200 w-1/3 rounded-md animate-pulse"
          />
        }
      ></PageHeader>
    </Page>
